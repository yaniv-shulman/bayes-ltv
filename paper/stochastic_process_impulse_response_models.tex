...
\]
where $R_{\mathcal{E}}(t; \tau, \tau') = \operatorname{Cov}[\mathcal{E}(t,\tau),\mathcal{E}^*(t,\tau') \mid \mathcal{D}]$ is the posterior covariance kernel of the process $\mathcal{E}(t,\tau)$ at fixed time $t$. This expression quantifies how uncertainty in the impulse response translates to uncertainty in the output.
...
\end{equation}
converges, under standard independence or ergodicity assumptions across realizations and provided $\E[\abs{R_1^{(T)}(f)}^2 \mid \mathcal{D}]<\infty$, to the finite-time ensemble spectrum
\begin{equation}
S_r^{(T)}(f)=\frac{1}{T}\E[\abs{R_1^{(T)}(f)}^2 \mid \mathcal{D}].
\end{equation}
This provides a natural spectral description for deterministic probing through an LTIE channel. If, in addition, the centered output process admits a wide-sense stationary infinite-time limit, then this finite-time spectrum reduces to the ordinary PSD. In the more general LTV case where $\mu(t,\tau)$ depends on $t$, the system exhibits non-stationary behavior in its mean, and time-frequency analysis methods are required to characterize the output spectrum. A proof is omitted, as the convergence follows directly from the law of large numbers applied pointwise in frequency to $\abs{R_m^{(T)}(f)}^2$.
...
\end{equation}
Here, the impulse response at time $n$ is a $p$-dimensional random vector $h[n]=(h_1[n], \ldots, h_p[n])^\top$, where $h_k[n]$ denotes the $k$-th tap at time $n$; equivalently, in elementwise notation we write the time-varying impulse response as $h[n,k] \equiv h_k[n]$. The summation begins at $k=1$, following a common convention in system identification where the direct feedthrough term ($k=0$) is often handled separately or assumed to be zero.
...
\end{equation}
This sesquilinear form shows how the input signal's structure probes the system's underlying uncertainty, as captured by $\Sigma[n]$.
...
\end{equation}
If the fluctuations are uncorrelated across time, i.e., $\mathbb{E}[ \mathcal{E}[n]\mathcal{E}[m]^H \mid \mathcal{D} ] = \delta_{n,m} \Sigma[n]$, the output signal $g[n]$ will have a non-stationary white stochastic component.

Under a local stationarity assumption, where the statistics of $h[n]$ vary slowly enough to be considered constant over a short-time analysis window, we can analyze the system's local spectrum. At a given time $n$, the expected power transfer function of the system is $\mathbb{E}[|H(n, e^{j\omega})|^2] = |\mu(n, e^{j\omega})|^2 + S_{\mathcal{E}}(n, e^{j\omega})$, where $\mu(n, e^{j\omega})$ is the DTFT of the mean response $\mu[n]$ and $S_{\mathcal{E}}(n, e^{j\omega})$ is the local power spectrum of the fluctuations at time $n$. For a deterministic input sequence with spectral magnitude $|F(e^{j\omega})|^2$, a corresponding local output spectrum is approximated by
\begin{equation}
\widetilde{S}_{gg}(n, e^{j\omega}) \approx | F(e^{j\omega}) |^2 \left( | \mu(n, e^{j\omega}) |^2 + S_{\mathcal{E}}(n, e^{j\omega}) \right).
\end{equation}
Here, $\widetilde{S}_{gg}(n, e^{j\omega})$ denotes a local finite-window spectral characterization rather than a global PSD.
...
As a foundational example, we demonstrate how the Bayesian machinery can robustly solve the classic problem of system identification from a single, noisy observation pair. While the theoretical sections analyzed systems that are inherently stochastic (LTV) or LTI-in-expectation, this experiment addresses a common precursor: how to infer the properties of a deterministic LTI system when they are unknown and masked by noise. This experiment is intentionally included to demonstrate that the same Bayesian framework remains useful even when the data are generated under the more conventional additive-noise model, rather than under a stochastic impulse-response model.
...
\end{equation}
where $\otimes$ is the cross-correlation operator and $(f \otimes f)$ is the autocorrelation of the input signal. Classically, one could attempt to estimate $h[n]$ by deconvolving the input autocorrelation from the observed CCF. However, direct deconvolution is often unstable and prone to noise amplification \citep{BerteroBoccacci2005}.
...
The final loss used in practice is $\text{Loss} = ||g - \hat{g}||_2^2 + \beta \cdot \text{KL}(q(h) || p(h))$, where $q(h)$ is the approximate posterior, $p(h)$ is the prior, and $\beta$ scales the KL divergence term according to the number of statistically distinct training examples represented in the objective. In the single-observation experiment, this number is $1$. Under a Gaussian observation model, this objective is proportional to the negative ELBO up to additive constants and a fixed noise-variance scaling of the squared L2 term.
...
In the standard experiment (without quantization), our Bayesian model (MIR) demonstrates superior performance, converging to a lower overall error floor compared to the classical CCF method. This demonstrates a substantial improvement in data efficiency, as illustrated in Figure~\ref{fig:ant_error_vs_num_pairs}, which plots the estimation error as a function of the number of pairs. Furthermore, with sufficient data, our method achieved a lower overall error floor, reaching a minimum error of approximately 0.213 compared to 0.268 for the classical approach.

This advantage was also observed in a more challenging scenario involving aggressive 1-bit quantization of the signals. While both methods proved robust, our Bayesian model again showed better performance. For example, with 10,000 pairs, our model's error was approximately 0.258, while the classical method's error was 0.316. Notably, the performance of the CCF method saturated quickly in this setting, showing little improvement with additional data, whereas our Bayesian approach continued to reduce its error as more pairs were included.
...
\end{equation}
where $\sum \alpha_i[n] = 1$ and $h[n]$ is the impulse response vector at time $n$. A single seismic signal from the "Earthquakes" dataset was used as the known input signal $f[n]$. The observed output $g[n]$ was generated by convolving $f[n]$ with the time-varying $h[n,k]$ and adding white Gaussian noise. The complete set of synthetic signals used is shown in Figure~\ref{fig:ltv_experiment1_example_pair}. It is important to note that this generative model, with additive noise, differs from the theoretical framework developed earlier, where the impulse response itself is stochastic.
...
\begin{figure}[!ht]
    ...
    \caption{Synthetic data for the LTV system identification experiment. The figure shows: the input signal $f[n]$, the ground-truth time-varying impulse response $h[n, k]$, and the resulting noisy output signal $g[n]$.}
